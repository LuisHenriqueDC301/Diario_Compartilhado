...
class LocalAuthService {
  ...
  static Future<bool> hasSupport({bool biometricOnly = true}) async {
    ...
    return canCheck ||
        hasSupport; //Caso algum deles for verdadeira, haverá suporte
  }

  static Future<bool> authenticate(
      {String? message, bool biometricOnly = true}) async {
    try {
      final isAuthenticated = await _auth.authenticate(
        localizedReason: message ??
            'Por favor, realize a autenticação biométrica', //Motivo da solicitação de autenticação
        ...
        authMessages: [
          const AndroidAuthMessages(
            ...
            goToSettingsDescription:
                'Não há nenhuma biometria cadastrada. Você deseja ir para as configurações e cadastrar uma nova?',
          ),
        ...
        options: AuthenticationOptions(
          biometricOnly:
              biometricOnly, //Se for true, aceitará apenas biometria/Face ID
          stickyAuth:
              true, //Se for true, caso o app vá para segundo plano enquanto estava tentando autenticar, ao retornar para primeiro plano, o app vai tentar retomar a autenticação
          useErrorDialogs:
              true, //Se for true, em alguns casos vai tentar exibir um Dialog com "tratamentos de erros" já pré-definidos (como não ter nenhuma biometria cadastrada no dispositivo, por exemplo)
          sensitiveTransaction:
              false, //Se for true, em casos onde o reconhecimento facial é utilizado para desbloqueio, ainda há uma confirmação por Dialog que o usuário deve confirmar
        ),
      );
